...
%<*Chapter6>
\maketitle{6}{Proteins: Crucial Components of All Body Tissues}%
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{6_protein_synthesis}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{6_protein_turnover_synthesis_and_breakdown}
	\caption{Protein Turnover: Synthesis and Breakdown}
	\label{fig:protein-turnover-synthesis-and-breakdown}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{6_role_of_proteins_in_fluid_balance}
	\caption{Role of Proteins in Fluid Balance}
	\label{fig:role-of-proteins-in-fluid-balance}
\end{figure}
...
\begin{itemize}
	...
	\begin{itemize}
		\item Pancreatic enzymes called \emph{proteases} complete the digestion of proteins into single amino acids
	\end{itemize}
\end{itemize}

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{6_protein_digestion}
	\caption{Protein Digestion}
	\label{fig:protein-digestion}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{6_protein_needs}
	\caption{Protein Needs}
	\label{fig:protein-needs}
\end{figure}
...
\begin{table}[H]
	\centering
	\caption{Terms and Definitions of a Vegetarian Diet}
	\label{tab:types-of-vegetarian-diets}
	\rowcolors{2}{rowmedgreen}{rowlightgreen}
	\begin{adjustbox}{max width=0.975\paperheight,center}
		\begin{tabular}{p{0.27\paperwidth} p{0.32\paperwidth} p{0.32\paperwidth}}
			\rowcolor{rowdarkgreen}%
			...
			Pescovegetarian & Similar to semivegitarian but excludes poultry & \textit{Pesco} means ``fish,'' the only animal source of protein in this diet\\
			Lacto-ovovegetarian & Vegetables, grains, nuts, fruits, legumes, dairy products (\textit{lacto}), and eggs \textit{(ovo)} & Excludes animal flesh and seafood\\
			Lacto-vegetarian & Similar to lacto-ovovegetarian but excludes eggs & Relies on milk and cheese for animal sources of protein\\
			Ovovegetarian & Vegetables, grains, nuts, fruits, legumes and eggs & Excludes dairy, flesh, and seafood products\\
			Vegan (also called strict vegetarian) & Only plant-based foods (vegetables, grains, nuts, seeds, fruits, legumes) & May not provide adequate vitamin $\mbox{B}_{12}$, zinc, iron, or calcium\\
			...
			Fruitarian & Only raw or dried fruit, seeds, nuts, honey, and vegetable oil & Very restrictive diet; deficient in protein, calcium, zinc, iron, vitamin $\mbox{B}_{12}$, riboflavin, and other nutrients\\
			\rowcolor{rowdarkgreen} & & \\
		\end{tabular}
	\end{adjustbox}
\end{table}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{6_vegetarian_diet}
	\caption{Vegetarian Diet}
	\label{fig:vegetarian-diet}
\end{figure}
...
\begin{table}[H]
	\centering
	\caption{Nutrients of Concern in a Vegan Diet}
	\label{tab:vegan-nutrient-concern}
	\rowcolors{2}{rowmedgreen}{rowlightgreen}
	\begin{adjustbox}{max width=0.975\paperheight,center}
		\begin{tabular}{p{0.24\paperwidth} p{0.32\paperwidth} p{0.32\paperwidth}}
			\rowcolor{rowdarkgreen}%
			\textbf{Nutrient} & \textbf{Functions} & \textbf{Nonmeat/Nondairy Food Sources}\\
			Vitamin $\mbox{B}_{12}$ & Assists with DNA synthesis; protection and growth of nerve fibers & Vitamin $\mbox{B}_{12}$--fortified cereals, yeast, soy products, and other meat analogs; vitamin $\mbox{B}_{12}$ supplements \\
			Vitamin D & Promotes bone growth & Vitamin D--fortified cereals, margarines, and soy products; adequate exposure to sunlight; supplementation may be necessary for those who do not get adequate exposure to sunlight \\
			Riboflavin (Vitamin $\mbox{B}_{2}$) & Promotes release of energy; supports normal vision and skin health & Whole and enriched grains, green leafy vegetables, mushrooms, beans, nuts, and seeds \\
			Iron & Assists with oxygen transport; involved in making amino acids and hormones & Whole-grain products, prune juice, dried fruits, beans, nuts, seeds, and leafy vegetables (such as spinach)\\
			Calcium & Maintains bone health; assists with muscle contraction, blood pressure, and nerve transmission & Fortified soy milk and tofu, almonds, dry beans, leafy vegetables, calcium-fortified juices, and fortified breakfast cereals \\
			Zinc & Assists with DNA and RNA synthesis, immune function, and growth & Whole-grain products, wheat germ, beans, nuts, and seeds \\
			\rowcolor{rowdarkgreen} & & \\
		\end{tabular}
	\end{adjustbox}
\end{table}

\indepth{Vitamins and Minerals}
...
\begin{table}[H]
	...
	\begin{adjustbox}{max width=0.95\paperwidth,center}
		\begin{threeparttable}
			\caption{Fat-Soluble Vitamins}
			\label{tab:fat-soluble-vitamins}
			\rowcolors{2}{rowmedgreen}{rowlightgreen}
			\begin{tabular}{*{5}{p{0.175\paperwidth} }}
				\rowcolor{rowdarkgreen}\textbf{Vitamin Name} & \textbf{Primary Functions} & \textbf{Recommended Intake*} & \textbf{Reliable Food Sources} & \textbf{Toxicity/Deficiency Symptoms}\\
				A (retinol, retinal, retinoic acid) & Required for ability of eyes to adjust to changes in light

				Protects color vision Assists cell differentiation Required for sperm production in men and fertilization in women

				Contributes to healthy bone

				Contributes to healthy immune system & RDA:

				Men: 900 $\mu$g/day

				Women: 700 $\mu$g/day

				UL: 3,000 $\mu$g/day & Preformed retinol: beef and chicken liver, egg yolks, milk

				Carotenoid precursors: spinach, carrots, mango, apricots, cantaloupe, pumpkin, yams & \textit{Toxicity}: Fatigue, bone and joint pain, spontaneous abortion and birth defects of fetuses in pregnant women, nausea and diarrhea, liver damage, nervous system damage, blurred vision, hair loss, skin disorders

				\textit{Deficiency}: Night blindness and xerophthalmia; impaired growth, immunity, and reproductive function\\
				D (cholecalciferol) & Regulates blood calcium levels
				...
				Adults aged 19-70: 15$\mu$g/day

				Adults aged $>$70: 20 $\mu$g/day

				UL 100 $\mu$g/day &
				Canned salmon and mackerel, milk, fortified cereals &
				\textit{Toxicity}: Hypercalcemia

				\textit{Deficiency}: Rickets in children, osteomalacia and/or osteoporosis in adults\\
				E (tocopherol) & As a powerful antioxidant, protects cell membranes, polyunsaturated fatty acids, and vitamin A from oxidation
				...
				\textit{Deficiency}: Hemolytic anemia; impairment of nerve, muscle, and immune function\\
				...
				\textit{Toxicity}: None known

				\textit{Deficiency}: Impaired blood clotting, possible effect on bone health
				\\
				\rowcolor{rowdarkgreen} & & & &
			\end{tabular}
			\begin{tablenotes}
				\small
				\item *RDA: Recommended Dietary Allowance; UL: upper limit; Al: Adequate Intake.
			\end{tablenotes}
		\end{threeparttable}
	\end{adjustbox}
\end{table}
% TODO: Get the rest of the tables for Chapter 6's In Depth
...
\begin{itemize}
	...
	\begin{itemize}
		\item Chemical form (e.g., absorption of heme iron from meats, fish, poultry is $\sim$25\%, whereas non-heme iron from plant products is $\sim$3--5\%)
